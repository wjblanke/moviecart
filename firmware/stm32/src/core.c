 ...
 *   $F136–$F13E  PhasePad (VBLANK off → first visible HMOVE)
 ...
 *   $F200+       RamKernel source (69 bytes, copied to $80)
 ...
 * After VBLANK, jmp $F136 burns the same leftover cycles as the working
 * title kernel (after RTS / FIELD stores), so the first visible HMOVE matches.
 *
 * RIOT after copy: kernel $80–$C4, pack $CA–$E9, FIELD $FB, AUDIDX $FC.
...
/*
 * ColdStart only ($F000–$F04A). Copies 69 bytes from $F200 → $80, jmp $80.
...
static const uint8_t mc_boot_rom[MC_OFF_BOOT_END + 1]
	...
	0x21, 0x85, 0x02, 0x85, 0x2a, 0xa2, 0x0c, 0xca, 0xd0, 0xfd, 0x85, 0x2b, 0xea, 0xea, 0xa2, 0x44,
...
static const uint8_t mc_oshead[] __attribute__((section(".ccmram"))) = {
	...
	0xf1, 0x95, 0xca, 0xe8, 0xbd, 0x40, 0xf1, 0x95, 0xca, 0xe8, 0xbd, 0x40,
	0xf1, 0x95, 0xca, 0xe8, 0x88, 0x85, 0x02, 0xd0, 0xd3, 0xa9, 0x00, 0x85,
	0x19, 0xbd, 0x40, 0xf1, 0x95, 0xca, 0xe8, 0xbd, 0x40, 0xf1, 0x95, 0xca,
...
/*
 * After VBLANK off. Last Play WSYNC + RTS + stx VBLANK/AUDIDX + jmp here
 * already burned 19 cycles; ldx #7 / nop / jmp $80 makes the first $F09E
 * land on the same cycle as the working title kernel (off by at most 1).
 */
static const uint8_t mc_phase_pad[] __attribute__((section(".ccmram"))) = {
	0xa2, 0x07, 0xca, 0xd0, 0xfd, 0xea, 0x4c, 0x80, 0x00
};

/*
 * RamKernel at RIOT $80. Play remaining OS 23/24 + VS 3 + VB 37 from pack
 * (A12 low). jmp $F136 for HMOVE phase. Even: 63 nibbles (drop 64th). Odd: 64.
 */
static const uint8_t mc_ram_kernel[] __attribute__((section(".ccmram"))) = {
	...
	0xe8, 0x20, 0xab, 0x00, 0xa9, 0x02, 0x85, 0x00, 0xa2, 0x03, 0x20, 0xab,
	0x00, 0x86, 0x00, 0xa9, 0x02, 0x85, 0x01, 0xa2, 0x25, 0x20, 0xab, 0x00,
	0x86, 0x01, 0x86, 0xfc, 0x4c, 0x36, 0xf1, 0xa5, 0xfc, 0x4a, 0xa8, 0xb9,
	0xca, 0x00, 0x90, 0x05, 0x4a, 0x4a, 0x4a, 0x4a, 0x2c, 0x29, 0x0f, 0x85,
	0x19, 0xe6, 0xfc, 0x85, 0x02, 0xca, 0xd0, 0xe7, 0x60
};
...
bus_dispatch(uint16_t cart_off)
{
	...
	if (cart_off >= MC_OFF_PHASEPAD &&
	    cart_off < MC_OFF_PHASEPAD + sizeof(mc_phase_pad)) {
		SET_DATA(mc_phase_pad[cart_off - MC_OFF_PHASEPAD]);
		EMULATE_DONE
	}
